 ...
 */
static TreeNodeACA* search(RBTree x, Type key)
{
	if (x == NULL)
		return NULL;
	if ( x->key == key)
 ...
 */
static ValType iterative_search(RBTree x, Type key)
{
...
{
	if (root)
		_pre_order_put_in_queue(root->node, queue);
	return queue;
}
...
void _pre_order_put_in_queue(RBTree tree, Queue *queue)
{
	if (tree != NULL)
	{
		AppendQueue(queue, tree->val);
		_pre_order_put_in_queue(tree->left, queue);
		_pre_order_put_in_queue(tree->right, queue);
	}
}
